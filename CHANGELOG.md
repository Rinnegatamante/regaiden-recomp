...
- **Fixed Built-in Cheats Not Working**:
  ...
  - Re-anchored all built-in cheats to the real, hardware-verified memory map extracted from the original Game Boy Color ROM and disassembly:
    ...
    - **Freeze Combat Reticle / Always Perfect Hit**: Detects active combat via reticle entity page at `$C22C` and `$CBCC`, locks the reticle position to the target center (`$72` midpoint or dynamic target entity `$C22D`), mirrors it to `$C2E9` and Bank 1 entity offset `0x8F`, syncs 12.4 fixed-point coordinates at offset `0x32..0x33` and screen sprite coordinates at `0x3A..0x3B` (`target_x + 0xB0` on reticle entity, `$CBCD`, and `$CBCE`), locks hit check outcome `$CBD3` to `0x00` (perfect/critical hit, never `$FF` miss), sets `$CBD4` to `0x01` (hit success), sets the critical hit flag bit 3 in `$CBCB`, zeroes the 9-byte hit lookup map at `$CBE0..$CBE8` in WRAM0, and sets `$C186` to 2.
    - **One-Hit Kill in Battle**: Re-targeted combat entity memory from Bank 4 to WRAM Bank 1 (`$C13A = 1`) where live battle entities are allocated. Directly targets active enemy slots referenced by `$C228..$C22B` as well as all active Bank 1 entity slots (`$D000..$DF00`, excluding reticle/target entities), setting enemy HP at entity offset `0x46` (the actual damage/death HP offset verified in `bank_00d.asm:6873, 6925`) from >1 to 1 while leaving dead enemies (`flags & 0x08` / 0 HP) untouched, ensuring any weapon attack immediately underflows HP to 0 and triggers instant death in 1 hit. Also scans Bank 4 entity slots for overworld compatibility.
