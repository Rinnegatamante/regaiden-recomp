...
- **Fixed Built-in Cheats Not Working**:
  ...
  - Re-anchored all built-in cheats to the real, hardware-verified memory map extracted from the original Game Boy Color ROM and disassembly:
    ...
    - **One-Hit Kill in Battle**: Hooked directly into the combat damage calculation instruction `SUB E` (`loc_0d_5f8e` in `Resident_Evil_Gaiden__USA__funcs_28.c`), forcing weapon damage to 255 (`0xFF`) whenever the cheat is enabled. Guarantees carry borrow and immediate death sequence on any weapon hit (knife, handgun, shotgun, grenade launcher, assault rifle, rocket launcher). Also maintains entity offset `0x46` clamping for background compatibility.
